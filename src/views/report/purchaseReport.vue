  ...
  <div>
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        <div class="title"><span>Report on High-Value Purchases in Next 12 Months 大額採購預測報告</span></div>
      </div>
      <div class="searchBar">
        <div class="searchItem">
          <div class="item">
            <span>PO value >: HK$</span>
            <el-input v-model="money" size="small" style="width: 80px" />
          </div>
          <div class="item">
          ...
          <el-button type="primary" size="small" @click="exchangeRateClick">Set Exchange Rate</el-button>
        </div>
      </div>
      <el-table
        v-loading="loading"
    ...
    <option-exchange-rate-dialog ref="optionExchangeRateDialog" />
  </div>
</template>

<script>
import moment from 'moment'
import { a4List, a4Update } from '@/api/table'
import OptionExchangeRateDialog from '@/views/report/modules/optionExchangeRateDialog'
moment.locale('zh-cn')

export default {
  name: 'PurchaseReport',
  components: { OptionExchangeRateDialog },
  data() {
    return {
      updateLoading: false,
      value1: [],
  ...
  methods: {
    ...
    exchangeRateClick() {
      this.$refs.optionExchangeRateDialog.showDialog()
    },
    upDateClick() { // 更新
      this.updateLoading = true
      a4Update({
        threshold: String(this.money)
      }).then(res => {
    ...
    exportClick(data) { // 自定义导出
      if (data.id) {
        const url = window._CONFIG.baseUrl + '/a4/export/' + data.id
        if (url) {
          window.open(url)
        }
      }
