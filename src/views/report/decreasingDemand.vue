  ...
  <div>
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        <div class="title"><span>Report on Decreasing Demand for Parts 零件需求減少報告</span></div>
      </div>
      <div class="searchBar">
        <div class="searchItem">
          <div class="item">
            <span>Decrease % threshold：</span>
            <el-slider ref="slider" v-model="sliderValue" style="width: 250px;margin-right: 10px" :max="100" :min="0" :step="5" />
...
import { a2List, a2Update } from '@/api/table'
moment.locale('zh-cn')
...
export default {
  ...
  methods: {
    ...
    exportClick(data) { // 自定义导出
      if (data.id) {
        const url = window._CONFIG.baseUrl + '/a2/export/' + data.id
        if (url) {
          window.open(url)
        }
      }
