  ...
  <div>
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        <div class="title"><span>Report on Recommended Stock Level 零件庫存報告</span></div>
      </div>
      <div class="searchBar">
...
import { a3List, a3Update } from '@/api/table'
moment.locale('zh-cn')
...
export default {
  ...
  methods: {
    ...
    exportClick(data) { // 自定义导出
      if (data.id) {
        const url = window._CONFIG.baseUrl + '/a3/export/' + data.id
        if (url) {
          window.open(url)
        }
      }
